...
Use spaces in the field to specify multiple keywords. Place {} around a set of words to form a phrase, <> inside the {} to use exact search, and () around a single word to specify a username. '$@' for all mentions, '$#' for all links. Wildcards (*) are supported.`;
...
function hasLink($message) {
    return $message.find('a').hasClass('link-fragment');
}
...
class ChatHighlightBlacklistKeywordsModule {
    ...
    onMessage($message, {user, timestamp, messageParts}) {
        ...
        if ((blacklistKeywords.includes('$@') && isMention($message)) || (blacklistKeywords.includes('$#') && hasLink($message))) {
            return this.markBlacklisted($message);
        }

        if ((highlightKeywords.includes('$@') && isMention($message)) || (highlightKeywords.includes('$#') && hasLink($message))) {
